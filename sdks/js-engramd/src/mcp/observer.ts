...
const MAX_WINDOW = parseInt(
  process.env.ENGRAM_AUTO_CAPTURE_WINDOW ?? "100",
  10
);
...
export class AutoCapture {
  ...
  constructor(vault: MemoryVault, maxWindow = MAX_WINDOW) {
    this.vault = vault;
    this.seen = new Set();
  ...
  wrapHandler(inner: ToolHandler): ToolHandler {
    const self = this;
    return async function (request: MCPCallRequest): Promise<ToolCallResult> {
  ...
  async observe(toolName: string, args: Record<string, unknown>): Promise<void> {
    // Disabled — bail early
    if (!this.enabled) return;

    // engram_* tools are explicit captures — don't double-capture
    if (toolName.startsWith("engram_")) return;
