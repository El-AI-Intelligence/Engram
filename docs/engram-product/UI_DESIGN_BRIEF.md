...
## 13. As Shipped (2026-08) — deviations from the mockups above

- **Onboarding wizard:** modal 5-step flow (welcome → three memory kinds →
  guided demo → capture your own → context preview). Shows on first visit
  (`localStorage.engram_onboarded`) or whenever the vault is empty;
  re-openable from Settings. The demo step loads 12 ready-made memories
  (project/tag `demo`, plus 4 links so the Graph has edges); removable in one
  click from Settings → Onboarding & Demo Data (purge by project `demo`).
- **Dashboard:** six stat cards (memories, QEM hit rate, new today, decayed
  last night, semantic, imagined), layer breakdown, vault health (QEM warm
  state, uptime, last decay/consolidation), strength-distribution chart
  (4 buckets from the 200 most recent memories), recent captures feed.
- **Settings:** schedule is interval-based (`decay_interval_hours` /
  `consolidation_interval_hours` + auto toggles), not calendar-based.
  Context defaults (budget, reserve, max engrams/turns) editable via
  `PATCH /config` (whole sub-object sent — the server replaces it).
  Embedding model panel is display-only. No rekey UI.
- **Privacy section (Settings):** `GET /privacy/audit` breakdowns by
  layer/source/project + danger-zone purge form (`POST /privacy/purge`,
  criteria: source/layer/project/before_date, ≥1 required) with a confirm
  modal and export-first escape.
